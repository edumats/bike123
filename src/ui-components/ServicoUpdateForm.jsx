...
export default function ServicoUpdateForm(props) {
  const {
    id: idProp,
    servico: servicoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = servicoRecord
      ? { ...initialValues, ...servicoRecord }
      : initialValues;
    setCep(cleanValues.cep);
    setEndereco(cleanValues.endereco);
    setNumero(cleanValues.numero);
    setComplemento(cleanValues.complemento);
    setBairro(cleanValues.bairro);
    setCidade(cleanValues.cidade);
    setEstado(cleanValues.estado);
    setReferencia(cleanValues.referencia);
    setObservacoes(cleanValues.observacoes);
    setTipo(cleanValues.tipo);
    setBike_marca_modelo(cleanValues.bike_marca_modelo);
    setCliente_email(cleanValues.cliente_email);
    setCliente_telefone(cleanValues.cliente_telefone);
    setCliente_nome(cleanValues.cliente_nome);
    setErrors({});
  };
  const [servicoRecord, setServicoRecord] = React.useState(servicoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Servico, idProp)
        : servicoModelProp;
      setServicoRecord(record);
    };
    queryData();
  }, [idProp, servicoModelProp]);
  React.useEffect(resetStateValues, [servicoRecord]);
  const validations = {
    cep: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Servico.copyOf(servicoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ServicoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Cep"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || servicoModelProp)}
