...
export default function MecanicoUpdateForm(props) {
  const {
    id: idProp,
    mecanico: mecanicoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = mecanicoRecord
      ? { ...initialValues, ...mecanicoRecord }
      : initialValues;
    ...
    setCnh(cleanValues.cnh);
    setImagem_cnh(cleanValues.imagem_cnh);
    setBiografia(cleanValues.biografia);
  ...
  const [mecanicoRecord, setMecanicoRecord] = React.useState(mecanicoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Mecanico, idProp)
        : mecanicoModelProp;
      setMecanicoRecord(record);
    };
    queryData();
  }, [idProp, mecanicoModelProp]);
  React.useEffect(resetStateValues, [mecanicoRecord]);
  const validations = {
    nome: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Mecanico.copyOf(mecanicoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MecanicoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nome"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mecanicoModelProp)}
